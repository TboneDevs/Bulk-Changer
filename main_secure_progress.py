...
async def secure_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Please upload the file containing accounts (format: email:password per line).\n\n⏱ While /secure is processing, you will receive status updates every 5 minutes showing how many accounts are done and how many remain.")
...
async def process_bulk_update(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(f"Processing {total_accounts} accounts and {len(targets)} target changes...\n\n⏱ Progress updates will be sent every 5 minutes.")
        ...
        for account in accounts:
            ...
            processed_count += 1
            remaining_count = max(total_accounts - processed_count, 0)

            if time.monotonic() - last_progress_update >= 300:
                await update.message.reply_text(
                    f"⏱ Secure update status:\n"
                    f"✅ Accounts done: {processed_count}/{total_accounts}\n"
                    f"⏳ Accounts remaining: {remaining_count}"
                )
                last_progress_update = time.monotonic()
